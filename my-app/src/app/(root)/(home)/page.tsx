...
export default function Home() {
  ...
  const [modalType, setModalType] = useState<"start" | "join" | null>(null);
  ...
  return (
    <div className="relative min-h-screen bg-slate-950 text-white overflow-hidden">
      ...
      <div className="container max-w-7xl mx-auto p-6">
        ...
        {/* Welcome */}
        <div className="rounded-2xl backdrop-blur-md bg-white/5 p-10 border border-white/10 shadow-2xl mb-14">
        ...
        {isInterviewer && (
          <>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {QUICK_ACTIONS
                .filter((action) => action.title !== "Find Job" && action.title !== "Tech Prep" && action.title !== "Mock Coding")
                .map((action) => (
                  <div
                    key={action.title}
          ...
          <>
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {QUICK_ACTIONS.filter(
                (action) =>
                  action.title === "Join Interview" ||
                  action.title === "Find Job" || action.title === "Tech Prep" || action.title === "Mock Coding"
              ).map((action) => (
                <div
                  key={action.title}
                  className="transition duration-300 transform hover:-translate-y-1 hover:shadow-xl"
